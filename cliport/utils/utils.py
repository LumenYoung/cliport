...
def matrix_to_heatmap_img(matrix) -> Image.Image:
    ...
    heatmap = cm.hot(normalized_matrix)

    # convert the heatmap (an RGBA image) to RGB and convert to a PIL image
    img = Image.fromarray((heatmap[:, :, :3] * 255).astype(np.uint8))
...
class PILLoader:
    ...
    def __init__(self, image: Image.Image):
        ...
        self._rowstride = self._width * self._channels  # width * number of channels
    ...
    def get_pixels(self) -> ctypes.Array:
        ...
        """
        # Convert the numpy array to ctypes array of c_uint8
        pixel_array = (ctypes.c_uint8 * len(self._pixels))(*self._pixels)
        return pixel_array
...
@display_image_in_cli.register(list)
@display_image_in_cli.register(tuple)
def _(img):
...
class ImageRotator:
    ...
    def __call__(self, x_list, pivot, reverse=False):
        ...
        for i, angle in enumerate(self.angles):
            ...
            _, _, h, w = x.shape
            x_warped: torch.tensor = warp_affine(
...
if __name__ == "__main__":
    img_fn = "/export/home/yang/git/thesis_experiment_repo/cliport/images_for_feedback/robot with alphabet blocks.png"

    img = Image.open(img_fn)
    display_image_in_cli(img)
